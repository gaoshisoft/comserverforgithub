...
Public Class TJWJXDataBlock
    ...
    Public Function GetValueFromRvData(ByVal L As Integer, ByVal v() As Byte) As Boolean _
        ...
        If Left(Rvs, 2) = CStr(Me.Addr) Then
            Rvs = Chr(&HA5) & Rvs
        End If
        If v(0) = &HA5 And CStr(Val("&H" & Mid(Rvs, 2, 2))) = CStr(Me.Addr) Then


            '--------------
            Dim LJLLs As String
            LJLLs = Mid(Rvs, 4, 10)
            Me.LJLL = Val(LJLLs)/100
            ...
            '--------------
            Me.PowerState = Val(Mid(Rvs, 31, 2))/10
            Mbs.WritevaluebyAd(Me.SvrDevAd, CStr(400000 + Me.SvrMBADStart + 9), Device.Datatype.浮点数, Me.PowerState)
            '-------------------------
            'Dim GKLJs As String
            'GKLJs = Mid(Rvs, 29, 6)
            'Me.GKLJ = Val(GKLJs)
