...
Friend Class MBTCPsocket
    ...
    Dim WithEvents Tmr5s As New Windows.Forms.Timer()
    ...
    Public Sub New(ByVal Ada As Int16, ByVal TcpPort As Integer)
        ...
        Me.Tcpport = TcpPort
        'Dim Ip As IPAddress
        'Ip = Dns.GetHostAddresses(Dns.GetHostName)(3)
    ...
    Sub Init(ByVal Ada As Int16, ByVal TcpPort As Integer)
        Me.InitializeComponent()
        If TcpClnt Is Nothing Then
            TcpClnt = New MyWinSockClient
        Else
            TcpClnt.TcpClnt.Close()
        End If
        Me.Tcpport = TcpPort
        'Dim Ip As IPAddress
        'Ip = Dns.GetHostAddresses(Dns.GetHostName)(Ada - 1)
        If TcpSvr Is Nothing Then
            TcpSvr = New MyWinSockListener(IPAddress.Any, TcpPort)
        Else
            TcpSvr.Stop()
            TcpSvr = Nothing
    ...
    Private Sub Tmr5s_Tick(ByVal sender As Object, ByVal e As System.EventArgs) Handles Tmr5s.Tick
        ...
        For i = ConnCol.Count - 1 To 0 Step -1
            ...
            If ConnCol(i).MyWaitTime >= 20 Then
                ...
                RemoveHandler TC.DataArrival, AddressOf Me.TcpClnt_DataArrival '必须先 解除委托关系，才能解除引用关系，才能最终销毁对象
                TC.StopMe()
                TC.TcpClnt = Nothing
                TC = Nothing
